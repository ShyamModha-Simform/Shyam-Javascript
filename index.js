...
class Model {
  ...
  setProductData(product) {
    try{
      if(isLocalStorageFull([...this.productData, product])){
        throw new Error("Storage Memory is Full! You can only store up to 5MB");
      }
  ...
  updateProductData(indexOfExistingProduct, product) {
    try{
      if(isLocalStorageFull([...this.productData, product])){
        throw new Error("Storage Memory is Full! You can only store up to 5MB");
      }
      console.log(this.productData, "=== updated product data===");
      this.productData.splice(indexOfExistingProduct, 1, product);
      localStorage.setItem("productData", JSON.stringify(this.productData));
    }catch(e){
      throw new Error(e.message);
    }
...
class View {
  ...
  onSubmitForm(addNewProduct) {
    addNewProduct.addEventListener("submit", (e) => {
      ...
      // To handler if memory is exceeded or not
      try{
    ...
    submitUpdates.addEventListener("click", () => {
      try{
        const productsBuffer = this.controller.updateProduct(
          ...
          this.renderCardsView(productsBuffer); 
        }catch(e){
          alert(e.message);
        }
    });
...
class Controller {
  ...
  updateProduct(product, id) {
    ...
    this.model.updateProductData(indexOfExistingProduct, product); // updating product list through Model
...
// Error checking logic -- Confused that in which class should i Put this function
// Or Should I create new Class named ErrorLogic..

function isLocalStorageFull(productsArray){
  const myDataString = JSON.stringify(productsArray);
  const encodedMyDataString = encodeURI(myDataString);
  const availableSpace = (5*1024*1024) - encodedMyDataString.length;
  if(availableSpace < 0){
    return true;
  }
  return false;
